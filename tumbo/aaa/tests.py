...
class CasLoginTestCase(BaseTestCase):
    ...
    def _setup(self):
        self.userland_home = reverse(
            'userland-static', args=['user1', self.base1.name, "index.html"])

        # Step 0: Try to log into the service, redirects to CAS Login with the service as parameter
        self.userland_ticketlogin = reverse(
            'userland-cas-ticketlogin', args=['ticket', 'service'])
