...
def update_job(apy, scheduler):
    """Sends schedule configuration for an Apy to the Scheduler Service. 
    
    Arguments:
        apy {Apy} -- Apy to update for.
        scheduler {[type]} -- Reference to Scheduler Service.
    """
    ...
    if apy.schedule:
        ...
        if scheduler.get_job(job_id):
            ...
            job_id = scheduler.add_job(call_apy, 'cron', args=[
                                       apy.base.name, apy.name], id=job_id, **kwargs)
...
def start_scheduler():
    ...
    scheduler = BackgroundScheduler(
        executors=executors1, jobstores=jobstores, job_defaults=job_defaults, timezone=utc)

    # Cleanup
    if hasattr(settings, "TUMBO_CLEANUP_INTERVAL_MINUTES"):
        job_id = scheduler.add_job(call_command, 'interval', minutes=int(
            settings.TUMBO_CLEANUP_INTERVAL_MINUTES), args=["cleanup_transactions"])
        logger.info(job_id)
    ...
    while True:
        ...
        for apy in Apy.objects.all():
            try:
                ...
                if not apy.schedule:
                    logger.exception("Problem with schedule config for %s: %s" % (
                        apy.name, apy.schedule))
