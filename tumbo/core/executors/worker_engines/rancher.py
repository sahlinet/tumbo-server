...
class RancherApiExecutor(BaseExecutor):
    ...
    def __init__(self, *args, **kwargs):
        self.auth = requests.auth.HTTPBasicAuth(
            settings.RANCHER_ACCESS_KEY, settings.RANCHER_ACCESS_SECRET)
    ...
    def _call_rancher(self, uri_appendix, data=None, force_post=False, full_url=None):
        ...
        if r.status_code == 422:
            logger.warning("422 status_code when calling %s, %s" %
                           (url, r.text))
    ...
    def start(self, id, *args, **kwargs):
        ...
        status_code, response = self._call_rancher(
            "/%s?action=activate" % id, force_post=True)
    ...
    def state(self, id):
        try:
            ...
            return False
        logger.info("Worker is in state: %s (transitioningMessage=%s)" %
                    (container['state'], container['transitioningMessage']))
        logger.debug("state(%s) returns: %s" %
                     (id, str(container['state'] == "active")))
